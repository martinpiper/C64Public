...
#include "../../sidplay-2.0.9/src/SimpleInterface.h"

MemoryHook* MemoryHook::mMemoryHook = 0;
...
void Player::writeMemByte_playsid (uint_least16_t addr, uint8_t data)
{
	...
	if (MemoryHook::mMemoryHook)
	{
		event_clock_t delta = context().getTime(mHookCounter,context().phase());
		mHookCounter += delta;
		mLargeHookCounter += delta;
		// MPi: TODO: This will need to detect NTSC or PAL SID timings
		double theTime = double(mLargeHookCounter) / CLOCK_FREQ_PAL;
		MemoryHook::mMemoryHook->SIDRegisterWrite(theTime,tempAddr & 0x1f,data);
	}
