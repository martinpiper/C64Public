Feature: Segment clip tests
  ...
  Scenario Outline: Segment extension test, existing segments, new segment has different colour and it clipped, new segment at second position.
    Given I have a simple overclocked 6502 system
    And I create file "t.a" with
      """
      kSpanPlot_maxHeight = 1
    ...
    Then I expect to see Segments_array+(0*kSegment_length)+kSegment_offset_nextLo equal lo(Segments_array+(3*kSegment_length))
    Then I expect to see Segments_array+(0*kSegment_length)+kSegment_offset_nextHi equal hi(Segments_array+(3*kSegment_length))
    Then I expect to see Segments_array+(0*kSegment_length)+kSegment_offset_left equal <existing left>
    Then I expect to see Segments_array+(0*kSegment_length)+kSegment_offset_right equal <existing right>
    Then I expect to see Segments_array+(0*kSegment_length)+kSegment_offset_colour equal 0

    Then I expect to see Segments_array+(3*kSegment_length)+kSegment_offset_nextLo equal lo(Segments_array+(1*kSegment_length))
    Then I expect to see Segments_array+(3*kSegment_length)+kSegment_offset_nextHi equal hi(Segments_array+(1*kSegment_length))
    Then I expect to see Segments_array+(3*kSegment_length)+kSegment_offset_left equal <expected left1>
    Then I expect to see Segments_array+(3*kSegment_length)+kSegment_offset_right equal <expected right1>
    Then I expect to see Segments_array+(3*kSegment_length)+kSegment_offset_colour equal 3
  ...
  Examples:
    | existing left | existing right   | existing left2 | existing right2 | existing left3 | existing right3  | new left | new right | expected left1 | expected right1 |
	| 10            | 20               | 30             | 35              | 40             | 50               | 25       | 28        | 25             | 28              |
	| 10            | 20               | 30             | 35              | 40             | 50               | 25       | 30        | 25             | 30              |
	| 10            | 20               | 30             | 35              | 40             | 50               | 25       | 32        | 25             | 30              |





  Scenario Outline: Segment extension test, existing segments, new segment has the same colour and extends one or both ends of some segments, no new segment
    Given I have a simple overclocked 6502 system
    And I create file "t.a" with
      """
      kSpanPlot_maxHeight = 1
